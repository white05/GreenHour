				...
				return this.each(function() {
					...
					if (opt.get!="true" && opt.get!=true) {
						...
						if (window.tplogs==true)
							try{
								console.groupCollapsed("Essential Grid  1.1.1 Initialisation on "+container.attr('id'));
						...
						if (opt.spinner=="spinner3" || opt.spinner=="spinner4") container.parent().find('.punchgs.Bounce1, .punchgs.Bounce2, .punchgs.Bounce3, .dot1, .dot2').css({backgroundColor:opt.spinnerColor});
...
*/
			...
			****************************************/
			jQuery('.esg-filter-wrapper').each(function() {
				var efw = jQuery(this);
			...
			******************************************/
			...
			jQuery(window).resize(function() {
					...
					if (navcont.length>0) {
						...
						punchgs.TweenLite.set(container.find('.esg-overflowtrick'),{width:container.width() - wcor});
					}
			...
			************************************************/
			...
			container.on('itemsinposition',function() {
				...
				if (opt.maxheight>0 && opt.maxheight<9999999999) {
					...
					punchgs.TweenLite.to(ul,heightspeed,{force3D:"auto",height:newheight,ease:punchgs.Power3.easeInOut});
					punchgs.TweenLite.to(esgo,heightspeed,{force3D:"auto",height:newheight,ease:punchgs.Power3.easeInOut});
	...
	item.find('.eg-handlehideunder').each(function() {
		...
		var hideunder = elem.data('hideunder');
			  if (item.width()<hideunder && hideunder!=undefined) {
				    elem.find('.eg-innerwrap-handlehideunder').css({visibility:'hidden'});							  
			  } else {
				  	elem.find('.eg-innerwrap-handlehideunder').css({visibility:'visible'});							  
			  }
	 });
  		 ...
  		 item.data('hovertimer',setTimeout(function() {
				 ...
				 if (item.find('.esg-entry-content').length>0 && art!="set" && opt.layout=="even") {
					...
					punchgs.TweenLite.set(item,{z:0.1,zIndex:50,x:0-(pl+pr+br+bl)/2, y:0-(pt+pb+bt+bb)/2,height:"auto",width:item.data('www')+pl+pr+bl+br});
					 ...
					 if (opt.inanimation != true ) punchgs.TweenLite.set(item.closest('.esg-overflowtrick'),{overflow:"visible"});
				 }
 ...
 **********************************/
 function prepareItemsInGrid(opt,appending) {
									  ...
									  if (maxdelay<dd) maxdelay = dd;
			  						  ...
			  						  switch (key[0]) {
			  						  		case ".esg-slide":
			  						  				...
			  						  				animto.x = xy.x;
			  						  				animto.y = xy.y;
													punchgs.TweenLite.to(animobject,0.5,{y:animto.y, x:animto.x,overwrite:"all",onComplete:function() {
														punchgs.TweenLite.set(animobject,{autoAlpha:0});
													}});
			  						  		break;
			  						  		case ".esg-slideout":
	...
	loadVideoApis(container,opt);
 	...
 	//Show Container
 	if (!appending) {
	 	punchgs.TweenLite.fromTo(ul,0.2,{autoAlpha:0,opacity:0},{force3D:"auto",autoAlpha:1,ease:punchgs.Power3.easeInOut});
	 	punchgs.TweenLite.fromTo(esgo,0.2,{autoAlpha:0,opacity:0},{force3D:"auto",autoAlpha:1,ease:punchgs.Power3.easeInOut});
		...
		if (img.data('lazysrc')!=img.attr('src') && img.data('lazysrc')!=undefined && img.data('lazysrc')!='undefined') {
			...
			if (img.data('lazysrc') !=undefined && img.data('lazysrc') !='undefined') {
				...
				limg.onload = function(i) {
					img.data('lazydone',1);
					punchgs.TweenLite.to(img.parent().find('.lazyloadcover'),0.3,{autoAlpha:0, onComplete:function() {
	...
	********************************/
	...
	if ((item.hasClass("itemtoshow") || item.hasClass("fitsinfilter")) && !p.skipanim) {
			...
			if (item.css("opacity")==0 && p.anim == "rotatescale")
				punchgs.TweenLite.set(item,{width:p.itemw,height:p.itemh,scale:1,left:localx,top:localy,x:"150%",y:"150%",rotationZ:20,overwrite:"all"});
		   ...
		   if (ecc && media) {
		  	...
		  	punchgs.TweenLite.to(item,p.speed,{force3D:"auto",autoAlpha:1,scale:1,transformOrigin:p.inorigin,rotationX:0,rotationY:0,y:0,x:0,z:0.1,rotationZ:0,left:localx,top:localy,ease:p.ease,delay:p.waits,
						onComplete:function() {
							if (item.hasClass("itemtoshow")) {
