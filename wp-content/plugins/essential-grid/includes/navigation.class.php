...
class Essential_Grid_Navigation {
	...
	public function __construct() {
		...
		$this->filter_dropdown_text = __('Filter Categories', EG_TEXTDOMAIN);
	 ...
	 */
	public function set_dropdown_text($text){
		
		$this->filter_dropdown_text = $text;
	...
	public function output_filter_unwrapped($demo = false, $type = ''){ //$type -> names what settings we need to check for: filter, filter2, filter3
		...
		$get_from = 'filter'.$type;
		$grouping = (isset($this->filter_settings[$get_from][$get_from.'-grouping'])) ? $this->filter_settings[$get_from][$get_from.'-grouping'] : 'false';
		$listing = (isset($this->filter_settings[$get_from][$get_from.'-listing'])) ? $this->filter_settings[$get_from][$get_from.'-listing'] : 'list';
		$do_show = $this->filter_settings[$get_from][$get_from.'-selected'];
        ...
        echo '<!-- THE FILTERING, SORTING AND WOOCOMMERCE BUTTONS -->';
		echo '<!-- THE FILTER BUTTONS -->';
        echo '<div class="esg-filter-wrapper'.$dropdown.'"';
		if($this->spacing !== false) echo $this->spacing;
		echo '>';
		
		if($listing == 'dropdown'){
			echo '<div class="esg-selected-filterbutton"><span>'.$this->filter_dropdown_text.'</span><i class="eg-icon-down-open"></i></div>';
