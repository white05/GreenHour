	...
	***********************/
	...
	t.initCreateGrid = function(doAction){
		...
		jQuery('select[name="post_category"]').change(function(){
			...
			jQuery('input[name="filter-selected[]"]:checked').each(function(){
				filter.push(jQuery(this).val());
			});

			var filter_all = []; //save the order
			jQuery('input[name="filter-selected[]"]').each(function(){
			...
			jQuery('.eg-filter-selected-order-wrap').html('');
			...
			for(var fa_key in filter_all){
				if(cur_selected !== null && cur_selected.indexOf(filter_all[fa_key]) > -1){ //still exists add it
					...
					opt_html = '<div class="eg-media-source-order revblue button-primary"><span style="float:left">'+jQuery('select[name="post_category"] option[value="'+filter_all[fa_key]+'"]').html()+'</span><input style="float:right;margin: 5px 4px 0 0;" class="eg-get-val" type="checkbox" name="filter-selected[]" value="'+filter_all[fa_key]+'" /><div style="clear:both"></div></div>';
					jQuery('.eg-filter-selected-order-wrap').append(opt_html);

					//now check if it should be selected
					if(filter !== null && filter.indexOf(filter_all[fa_key]) > -1) jQuery('input[name="filter-selected[]"][value="'+filter_all[fa_key]+'"]').attr('checked', 'checked');
			...
			if(cur_selected !== null){
				for(var key in cur_selected){
					...
					opt_html = '<div class="eg-media-source-order revblue button-primary"><span style="float:left">'+jQuery('select[name="post_category"] option[value="'+cur_selected[key]+'"]').html()+'</span><input style="float:right;margin: 5px 4px 0 0;" class="eg-get-val" type="checkbox" name="filter-selected[]" value="'+cur_selected[key]+'" /><div style="clear:both"></div></div>';
					jQuery('.eg-filter-selected-order-wrap').append(opt_html);

					if(!filter_startup){
						jQuery('input[name="filter-selected[]"][value="'+cur_selected[key]+'"]').attr('checked', 'checked');
					}
				}
			}
	...
	**/
	...
	t.get_navigation_layout = function(){
		...
		if(c_filter2.closest('.eg-navigation-drop-wrapper').attr('id') !== undefined){
			...
			order = c_filter2.parent().children('span').index(c_filter2);
		...
		if(c_filter3.closest('.eg-navigation-drop-wrapper').attr('id') !== undefined){
			...
			order = c_filter3.parent().children('span').index(c_filter3);
