...
// Extract tab titles
preg_match_all( '/vc_tab title="([^\"]+)"(\stab_id\=\"([^\"]+)\")(\stitle_icon\=\"([^\"]+)\"){0,1}/i', $content, $matches, PREG_OFFSET_CAPTURE );
$tab_titles = array();
...
foreach ( $tab_titles as $tab ) {
    ...
    preg_match('/title="([^\"]+)"(\stab_id\=\"([^\"]+)\")(\stitle_icon="([^\"]+)"){0,1}/i', $tab[0], $tab_matches, PREG_OFFSET_CAPTURE );

    if ( isset($tab_matches[4][0]) ) {
    	preg_match('/title="([^\"]+)"(\stab_id\=\"([^\"]+)\") title_icon="([^\"]+)"{0,1}/i', $tab[0], $tab_matches, PREG_OFFSET_CAPTURE );
    }
    ...
    if(isset($tab_matches[1][0])) {
        ...
        if ( isset($tab_matches[4][0]) ) {
        	$tabs_nav .= '<i class="'.$tab_matches[4][0].'"></i> ';
    	}
...
if ( 'vc_tour' == $this->shortcode) {
    $output .= "\n\t\t\t" . '<div class="wpb_tour_next_prev_nav clearfix"> </div>';
}
